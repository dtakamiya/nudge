...
- [x] メンバー別の1on1実施間隔をヒートマップまたはカレンダービューで表示 → `MeetingIntervalTable` として実装済み
- [x] 月ごとの実施回数のバーチャート（Recharts 等） → `MeetingFrequencyChart` として実装済み
- [x] ダッシュボードに「推奨: 今週1on1すべきメンバー」のレコメンド表示 → `RecommendedMeetingsSection` として実装済み
- [x] 新規ページ `src/app/analytics/page.tsx` の追加 → 実装済み
...
#### 2.4.7 マネージャー向け 1on1 アシスト機能（コーチングUI） — 優先度: 高

- [ ] ミーティング画面に「傾聴・質問」を促すフレーズ集や、ベストプラクティスのTips（「メンバーに7割話してもらいましょう」等）をサイドパネルに表示
- [ ] 承認・ねぎらいの言葉かけなどをリマインドし、質の高い対話をシステム側から自然にサポート
- [ ] （将来構想）対話の偏りを防ぐためのシンプルなタイマー機能（目安時間の提示）
...
- [x] 「最近のアクティビティ」タイムライン（直近の1on1記録、アクション完了のフィード） → `RecentActivityFeed` として実装済み
- [x] 「今日 / 今週のタスク」セクション（期限が近いアクションアイテム一覧） → `UpcomingActionsSection` として実装済み
- [x] メンバーカードのクイックアクション（ダッシュボードから直接1on1を開始） → 既存の「1on1」ボタン（`/meetings/new` リンク）が対応

#### 2.6.3 ミーティング準備画面の改善 — 優先度: 中

- [x] 前回のミーティングだけでなく、過去複数回のミーティングを参照可能に → `PastMeetingsAccordion` でアコーディオン形式（最大5件）表示
- [x] 前回の話題をワンクリックで今回のアジェンダにコピー → 各話題横の「コピー」ボタンで `handleCopyTopic` を実行
- [x] アクションアイテムの進捗確認チェックリスト（準備画面で事前にステータスを更新可能） → `PrepareActionChecklist` で `useOptimistic` + ステータスサイクル実装

#### 2.6.4 チェックイン＆クロージングフローの導入 — 優先度: 高

- [ ] **チェックイン**: ミーティング冒頭で、アイスブレイク用の話題サジェストや、メンバーのコンディション（体調・気分・業務量など）を簡単に記録できる UI の追加。心理的安全性の確保を自然に促す。
- [ ] **クロージング**: ミーティング終了時に、「次回に向けたアクションアイテムは設定されていますか？」と確認するステップ（ダイアログやサマリー画面）を挟み、やりっぱなしを防ぐ行動変容を促す。
...
### Phase 3: 差別化機能（1on1ベストプラクティスの実践支援）

- [ ] ミーティングごとのチェックイン＆クロージングフロー（アイスブレイク・アクション設定の仕組み化）
- [ ] マネージャー向け 1on1 アシスト機能（傾聴・質問のTipsやフレーズ集表示）
- [ ] リアルタイム記録モード
- [ ] 定期スケジュール設定
- [ ] 感情トラッキング
- [x] ミーティング準備画面の改善
